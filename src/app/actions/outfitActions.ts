...
export async function createOutfit(name: string, topId: number, bottomId: number, shoeId: number) {
  try {
    const stmt = db.prepare("INSERT INTO outfits (name, top_id, bottom_id, shoe_id) VALUES (?, ?, ?, ?)");
    stmt.run(name, topId, bottomId, shoeId);
  ...
  try {
    const stmt = db.prepare(`
      SELECT
        ...
        outfits.tags
      FROM outfits
      INNER JOIN clothing AS clothing_top ON outfits.top_id = clothing_top.id
      INNER JOIN clothing AS clothing_bottom ON outfits.bottom_id = clothing_bottom.id
      INNER JOIN clothing AS clothing_shoe ON outfits.shoe_id = clothing_shoe.id
    `);

    const outfits = stmt.all().map((row: any) => ({
      id: row.id.toString(),
      name: row.name,
      top: {
        id: row.top_id,
...
export async function updateOutfitTags(outfitId: string, tags: string[]) {
  try {
    const stmt = db.prepare("UPDATE outfits SET tags = ? WHERE id = ?");
    stmt.run(tags.join(','), outfitId); // Join the tags array into a comma-separated string
    revalidatePath('/outfits');
  } catch (error) {
